...
public:
    ...
    bool    operator<     (const String&)           const;
    String  substr        (int, int)                const;  //The sub-string from start to end
    int     findch        (int,  char)              const;  //Find location of char from a pos
    int     findstr       (int,  const String&)     const;  //Find location of str from a pos
    int     convertToNum  ();

    friend  std::ostream& operator<<(std::ostream&, const String&);
    friend  std::istream& operator>>(std::istream&, String&);
    friend  std::istream& getline(std::istream&, String&);
    std::vector<String> split(char) const;                  //Splits string and returns a vector of substrings

private:
            String        (int);                            //String(10) - size 10
